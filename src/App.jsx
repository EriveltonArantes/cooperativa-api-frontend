...
const apiBaseUrl = "https://cooperativa-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Cooperado</li>
          <li>✓ Controle completo de Produto</li>
          <li>✓ Controle completo de Entrega Produto</li>
          <li>✓ Controle completo de Dividendo</li>
        </ul>
      </div>
...
function PainelCooperado({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "municipio", "dataAdesao", "ativo", "quota", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/cooperados", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cooperados/" + editId : apiBaseUrl + "/api/cooperados";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "municipio", "dataAdesao", "ativo", "quota"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cooperados/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Cooperado")}</div>
            ...
            <div className="item-field"><b>quota</b><span>{item["quota"] != null ? Number(item["quota"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Cooperado</h3>
      ...
      <input type="number" step="0.01" value={form["quota"] ?? ""} onChange={e => setForm({...form, quota: e.target.value})} />
              <div className="modal-actions">
...
function PainelProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "tipo", "descricao", "unidade", "precoBruto", "precoLiquido", "estoque", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "tipo", "descricao", "unidade", "precoBruto", "precoLiquido", "estoque"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>precoBruto</b><span>{item["precoBruto"] != null ? Number(item["precoBruto"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>precoLiquido</b><span>{item["precoLiquido"] != null ? Number(item["precoLiquido"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            ...
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.precoBruto)}>Cobrar via Pix</button>
      ...
      <label className="field-label">precoBruto</label>
      <input type="number" step="0.01" value={form["precoBruto"] ?? ""} onChange={e => setForm({...form, precoBruto: e.target.value})} />
      <label className="field-label">precoLiquido</label>
      <input type="number" step="0.01" value={form["precoLiquido"] ?? ""} onChange={e => setForm({...form, precoLiquido: e.target.value})} />
...
function PainelEntregaProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [cooperadoList, setCooperadoList] = React.useState([]);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["cooperadoId", "produtoId", "data", "quantidade", "valorTotal", "qualidade", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/entregaprodutos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/cooperados", { headers: authHeaders(token) }).then(r => r.json()).then(setCooperadoList).catch(() => {});
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/entregaprodutos/" + editId : apiBaseUrl + "/api/entregaprodutos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["cooperadoId", "produtoId", "data", "quantidade", "valorTotal", "qualidade", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["cooperado", "produto"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/entregaprodutos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["qualidade"] ?? "EntregaProduto")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/entregaprodutos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["qualidade"] ?? "EntregaProduto")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>cooperadoId</b><span>{item["cooperadoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>cooperado</b><span>{item.cooperadoId ? ((cooperadoList.find(o => o.id === item.cooperadoId) || {}).nome ?? ("#" + item.cooperadoId)) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} EntregaProduto</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">cooperadoId</label>
      <input type="number" value={form["cooperadoId"] ?? ""} onChange={e => setForm({...form, cooperadoId: e.target.value})} />
      ...
      <input type="text" value={form["qualidade"] ?? ""} onChange={e => setForm({...form, qualidade: e.target.value})} />
      ...
      <label className="field-label">cooperado</label>
      <select value={form["cooperado"] ?? ""} onChange={e => setForm({...form, cooperado: e.target.value})}>
        <option value="">Selecione...</option>
        {(cooperadoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
...
function PainelDividendo({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["cooperadoId", "periodo", "percentualDistribuicao", "valor", "status", "dataPagamento", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/dividendos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/dividendos/" + editId : apiBaseUrl + "/api/dividendos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["cooperadoId", "periodo", "percentualDistribuicao", "valor", "status", "dataPagamento"].forEach(k => { f[k] = item[k] ?? ""; });
    ["cooperado"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/dividendos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
    ...
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["periodo"] ?? "Dividendo")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/dividendos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">D</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/dividendo" + item.id + "/320/200"} alt="Dividendo" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["periodo"] ?? "Dividendo")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>cooperadoId</b><span>{item["cooperadoId"] ?? "—"}</span></div>
            <div className="item-field"><b>percentualDistribuicao</b><span>{item["percentualDistribuicao"] != null ? Number(item["percentualDistribuicao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>dataPagamento</b><span>{item["dataPagamento"] ? new Date(item["dataPagamento"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>cooperado</b><span>{item.cooperadoId ? ((cooperadoList.find(o => o.id === item.cooperadoId) || {}).nome ?? ("#" + item.cooperadoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Dividendo</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">cooperadoId</label>
      <input type="number" value={form["cooperadoId"] ?? ""} onChange={e => setForm({...form, cooperadoId: e.target.value})} />
      <label className="field-label">periodo</label>
      <select value={form["periodo"] ?? "MANHA"} onChange={e => setForm({...form, periodo: e.target.value})}>
        <option value="MANHA">MANHA</option>
        <option value="TARDE">TARDE</option>
        <option value="NOITE">NOITE</option>
        <option value="INTEGRAL">INTEGRAL</option>
      </select>
      <label className="field-label">percentualDistribuicao</label>
      <input type="number" step="0.01" value={form["percentualDistribuicao"] ?? ""} onChange={e => setForm({...form, percentualDistribuicao: e.target.value})} />
      ...
      <label className="field-label">dataPagamento</label>
      <input type="datetime-local" value={form["dataPagamento"] ?? ""} onChange={e => setForm({...form, dataPagamento: e.target.value})} />
      <label className="field-label">cooperado</label>
      <select value={form["cooperado"] ?? ""} onChange={e => setForm({...form, cooperado: e.target.value})}>
        <option value="">Selecione...</option>
        {(cooperadoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Cooperado");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Cooperativa<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Cooperado" ? " active" : "")} onClick={() => setAba("Cooperado")}><span className="nav-ico">🗂️</span>Cooperado</button>
        <button className={"nav-btn" + (aba === "Produto" ? " active" : "")} onClick={() => setAba("Produto")}><span className="nav-ico">📦</span>Produto</button>
        <button className={"nav-btn" + (aba === "EntregaProduto" ? " active" : "")} onClick={() => setAba("EntregaProduto")}><span className="nav-ico">📦</span>EntregaProduto</button>
        <button className={"nav-btn" + (aba === "Dividendo" ? " active" : "")} onClick={() => setAba("Dividendo")}><span className="nav-ico">🗂️</span>Dividendo</button>
      ...
      <main className="main">
        ...
        {aba === "Cooperado" && <PainelCooperado token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Produto" && <PainelProduto token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "EntregaProduto" && <PainelEntregaProduto token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Dividendo" && <PainelDividendo token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
